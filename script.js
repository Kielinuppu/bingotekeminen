...
const kysymysAudio = new Audio('kysymys.mp3'); // Uusi äänitehoste
...
function aloitaArvonta() {
    if (seuraavaNappi.textContent === 'VALMIS!') {
        return; // Peli on päättynyt, ei tehdä mitään
    }
    ...
    kysymysAudio.play(); // Soitetaan kysymys.mp3

    setTimeout(() => {
        kysymysAudio.pause(); // Pysäytetään kysymys.mp3
        kysymysAudio.currentTime = 0; // Nollataan äänitiedoston toisto
        arvontaAudio.play();
        arvoSeuraava();
...
// Loput funktiot pysyvät ennallaan...
...
function arvoSeuraava() {
    ...
    arvottuKuva.src = arvottuKohde;
    arvottuKuva.alt = `Kuva ${arvottuIndeksi + 1}`;

    nykyinenAudio = aanet[arvottuIndeksi];

    lisaaArvottuKuvake(arvottuKohde, arvottuIndeksi + 1);
}
